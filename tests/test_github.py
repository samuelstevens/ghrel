...
def test_install_downloads_api_url_when_available(
    ...
    """Authenticated release asset downloads use the GitHub API asset URL."""
...
def test_install_downloads_browser_url_without_token(
    tmp_path: pathlib.Path, monkeypatch
) -> None:
    """Unauthenticated public downloads should avoid REST API quota."""
    package = ghrel.packages.PackageConfig(
        name="tool",
        pkg="owner/repo",
        binary={},
    ...
    ghrel.install.install_release_asset(
        package,
        release,
        asset,
        binary_pattern=None,
